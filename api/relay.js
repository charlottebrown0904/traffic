...
const ALLOW = {
  ...
  "www.data.go.kr":  {},
  // 토지·주택가격비준표 열람 서비스 (한국부동산원). 개별공시지가 산정에
  // 쓰는 **공식 가격배율표** 가 시군구 × 용도지역별로 있다 —
  // 도로접면·형상·고저·면적 등 22개 항목이다 (docs/appraisal-accuracy.md
  // §1-1). 우리 개별요인 지수표는 평가서 40건을 손으로 읽어 만든 값이라,
  // 이 표가 그 자리를 관(官)의 기준값으로 바꾼다.
  //
  // 키가 없는 공개 열람 서비스라 통과만 시킨다. 사람이 화면에서 시군구를
  // 하나씩 골라 엑셀을 받으면 229번을 눌러야 한다 — 그래서 러너가
  // 대신 돌게 열어 둔다. 어떤 요청으로 표가 나오는지는
  // scripts/page_dump.py 로 먼저 본다.
  "sct.reb.or.kr":   {},
