/* QR 부호기 — 의존성 없이, 우리 서버에서.
 *
 * 왜 직접 만드나. 외부 QR 서비스에 주소를 넘기면 **우리가 어디에 무엇을
 * 뿌리는지가 남의 서버 로그에 남는다.** 광고 계획은 그 자체로 정보다.
 * 그리고 그 서비스가 문 닫으면 인쇄해 둔 QR 이 통째로 죽는다.
 *
 * 범위를 좁혔다 — **바이트 모드 · 오류정정 M · 버전 1~10.**
 * 우리가 넣는 것은 toji.fyi/l/<코드> 뿐이라 서른 자 남짓이고, 그 길이는
 * 버전 3(29×29)이면 담긴다. 범위를 좁히면 표가 작아지고 틀릴 자리가 준다.
 *
 * 맞는지는 눈으로 안 본다 — scripts/test_qr.js 가 널리 쓰이는 부호기와
 * **모듈 하나하나를 맞춰 본다.** 찍히지 않는 QR 은 없는 것보다 나쁘다.
 */
(function () {
  'use strict';
  ...
  function applyFormat(g, mask) {
    ...
    /* **놓는 차례는 MSB 부터다.** 맨 처음 자리에 14번 비트가 간다.
       LSB 부터 놓으면 열다섯 칸이 통째로 뒤집혀, 그려지기는 하는데
       어떤 기계도 못 읽는 QR 이 된다 — 대조 부호기와 맞춰 보고 잡았다. */
    ...
    /* 둘째 사본. 아래 세로줄은 **일곱 칸**이다 — 여덟째 자리는 어두운
       모듈이 차지하고 있어서, 여덟 번째로 놓는 비트는 오른쪽 가로줄의
       맨 왼쪽 (8, size-8) 으로 간다. */
