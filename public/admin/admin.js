...
(function () {
  ...
  function bijunpyoBlock(v) {
    ...
    return '<h4>관의 공식 배율과 나란히 — 토지가격비준표</h4>'
      ...
      + '읍·면마다 배율이 다릅니다 — 어느 한 장이 "정답"은 아니고, 차이가 곧 개별공시지가와 감정평가의 거리입니다.</p>'
    ...
    ['값을 어떻게 내는가', '그 밖의 요인 — 두 갈래와 합치는 법',
      ...
      function (c) {
        ...
        return kpi([
          ...
          + '<p>한쪽만 있으면 그 값을 그대로 씁니다. 거래사례의 창은 <b>도시지역 ' + N(v.trade_years) + '년 · 관리·농림 '
          + N(v.trade_years_nonurban || v.trade_years) + '년</b>입니다 — 감정평가 실무기준 [610-1.5.2.5] ③4 가 그렇게 가릅니다 '
          + '(녹지지역은 국토계획법상 도시지역이라 3년 쪽입니다). 취소 제외, 거래면적이 필지면적의 0.5~2배인 것만 씁니다 — '
          + '지번 지오코딩이 옆 필지에 떨어진 거래를 걸러내는 장치입니다.</p>'
    ...
    ['값을 어떻게 내는가', '표준지를 고르는 벌점',
      ...
      function (c) {
        ...
        return '<h4>후보에서 빼는 것</h4>'
          ...
          + '<li>도로·형상·지세가 다를수록 벌점 가산</li>'
          + '<li><b>가격 수준</b> — 표준지 공시지가 ÷ 대상 개별공시지가가 띠를 벗어나면 벌점 가산 (벗어난 정도의 로그에 비례)</li></ul>'
          + '<p>가격 수준 줄은 좌표가 없어 위치 차이를 못 보는 자리를 개별공시지가가 대신 채우는 것입니다. '
          + '조건이 전부 같아도 공시지가가 몇 배 차이 나는 표준지는 다른 가격권이라는 뜻이고, 그것을 고르면 그 밖의 요인을 두 번 곱한 셈이 됩니다. '
          + '띠와 벌점 크기는 평가서 원장에서 평가사가 실제로 고른 표준지의 배율 분포로 정했습니다.</p>'
