...
4. 주소 검색(`mode=geocode`, 2026-09-11)도 이 함수다 — 브이월드 지오코더를 대신 부르고
   지번(PARCEL) → 도로명(ROAD) 순으로 묻는다. **수집 지오코딩과 같은 키의 하루 한도**를
   쓰므로 필지 조회와 같은 좁은 속도 제한(분당 120/IP)을 받고, 찾은 주소는 한 달 캐시한다.
