...
4. 주소 검색(2026-09-11)도 이 함수다. 두 길이 있다 — `mode=pnu` 는 명부의 법정동코드와
   지번으로 만든 PNU 로 연속지적도(WFS, OGC FILTER)에서 그 필지 하나를 받고, `mode=geocode`
   는 브이월드 지오코더(지번 PARCEL → 도로명 ROAD)다. 지오코더는 주소 DB 라 **건물 없는
   땅의 지번을 모른다**(실측: 건업리 140 → 140-1 로 뭉갬) — 그래서 PNU 가 먼저고 지오코더는
   명부에 코드가 없을 때의 뒷길이다. 둘 다 **수집 지오코딩과 같은 키의 하루 한도**를 쓰므로
   필지 조회와 같은 좁은 속도 제한(분당 120/IP)을 받고, 찾은 것은 한 달 캐시한다.
   탐침: `scripts/pnu_probe.py` (러너 워크플로 `probe.yml`) — CQL_FILTER 는 조용히 무시되어
   엉뚱한 첫 필지들이 오니 쓰면 안 된다.
