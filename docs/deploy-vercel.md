...
| **한계** | "IC 교통량이 늘면 땅값이 오른다"는 **아직 확인되지 않았다**고 적습니다 |
| 이용 | 가입은 관리자 승인제 |

한계 섹션을 뺄 수 없는 이유 — 판정표(`가설 판정` 탭)가 세 가설 모두
'아직 모름' 입니다. 첫 화면이 그보다 더 말하면 화면 안팎이 다른 말을 하게
됩니다. 그리고 '아직 모름' 과 '효과 없음' 은 다르다는 것도 같이 적습니다.
