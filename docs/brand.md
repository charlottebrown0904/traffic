...
**토지맥**. 도메인은 `toji.fyi`. 화면·문서·manifest 가 모두 이 이름입니다.

**맥(脈)은 도로망입니다.** 이 서비스의 가설이 "교통이 값을 만든다" 이므로,
이름이 그 가설을 한 글자로 말합니다. 마크가 이미 그 그림입니다 — 필지를
도로가 가로질러 뻗어 나갑니다.
...
관공서로 읽힙니다. `맥` 은 관청이 쓰지 않는 말이라 그 선을 넘지 않습니다 —
