...
## 이전 완료 — 확인된 것만 적는다 (2026-09-14 14:20 UTC)

| 확인한 것 | 결과 |
|---|---|
| 버킷에 올라갔나 | 435개 · 65.1MB |
| 열쇠 없이 받아지나 | meta 4KB · chart 415KB · trades-2025 1,463KB · places 2,004KB |
| **브라우저가 받을 수 있나 (CORS)** | **`access-control-allow-origin: *`** — 네 파일 모두 |
| 배포가 서나 | **READY** (4b14396) |
| 배포에서 자료가 빠졌나 | `/app/data/meta.json` → **404** |
| 배포된 app.js 가 버킷을 보나 | `DATA_BUCKET` 있음 · `fetchData()` 16곳 · `/app/data` 직접 호출 **0** |
| 배포 크기 | 65MB → **1.5MB** |

**CORS 를 따로 확인한 까닭.** 화면이 이제 다른 출처로 부른다
(toji.fyi → supabase.co). 머리글이 없으면 **curl 은 되는데 브라우저만
막힌다** — 앞의 '열쇠 없이 받아지나' 는 curl 쪽이라 그 둘을 못 가른다.
Origin 을 붙여 물어야 같은 자리에서 보는 것이다.

**하마터면 안 볼 뻔했다.** 사본을 .gitignore 로 뺀 순간 upload_only 일감이
올릴 것이 없어져 '없습니다' 하고 조용히 끝났는데, 작업은 **success** 로
찍혔다. 초록불을 보고 넘어갔으면 CORS 를 영영 안 봤을 것이다. 그래서
`--verify-only` 를 따로 뒀다 — 올릴 것이 없어도 버킷은 본다.

## 아직 모르는 것

**사용량 화면의 26.76GB 가 실제로 내려가는지는 아직 모른다.** 배포 크기가
줄어든 것은 확인했지만, 그 수치가 지연 반영인지 청구 기간 누적인지는
끝내 확정하지 못했다. 다음에 화면을 볼 때 확인할 일이다.
