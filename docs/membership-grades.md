...
| 코드 | 이름 (화면) | 뜻 | 프리미엄(현재 가치·미래 가치) | 등급 변경 |
|---|---|---|---|---|
| admin | 관리자 | 모든 권한 | 본다 | **할 수 있다** (남의 등급) |
| A | **VIP** | 제한 없음 — 결제 없이 열어 주는 계정 | 본다 | 못 한다 |
| B | **일반** | 유료. `grade_until` 이 있으면 그날까지 | 기간 안에서 본다 | 못 한다 |
| C | **손님** | 가입 기본값. **로그인 없이 들어온 사람**의 자리이기도 하다 | 못 본다 — 누르면 안내 | 못 한다 |

이름은 2026-09-12 지시로 바꿨다 (A→VIP · B→일반 · C→손님). 손님은 로그인 없이
들어온 사람을 가리킨다 — 지금은 가입이 필수라 비어 있지만, 나중에 로그인 없이
들어올 수 있게 풀면 그 사람이 이 자리에 온다. 2026-09-11 의 '프리미엄·일반' 은 옛 이름이다. 코드값은 데이터베이스
...
내 계정(/account) → 아래 **회원 목록** (관리자에게만 보인다 — 자물쇠는 profile 의
RLS 라 관리자가 아니면 남의 행이 아예 안 온다). 한 표에서 승인·거절과 등급 변경을
한다: 등급을 고르고, 프리미엄이면 만료일을 적고 → 저장. 바꾼 내역은 `grade_log`
에 남는다. 상태 칩(전체·승인 대기·이용 중·거절)·등급·검색으로 거르고, 열 머리를
눌러 정렬한다 (신청일 = 가입 신청 순간, 가입일 = 승인한 순간 `approved_at`).
'메일 복사' 는 보이는 회원의 메일을 쉼표로 이어 복사하고, 'CSV 내려받기' 는 같은
목록을 파일로 준다 (2026-09-11).
