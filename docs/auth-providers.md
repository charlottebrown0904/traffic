...
> 2026-09-16 갱신. 도메인이 `sado-toji.vercel.app` → **`toji.fyi`** 로,
> 서비스 이름이 `사도 토지` → **`토지랩`** 으로 바뀌었습니다. 옛 도메인은
> 2026-09-10 에 완전히 끊겼습니다 — 그 주소로는 이제 아무것도 안 열립니다.
> 아래 값은 전부 새 도메인·새 이름으로 고쳤습니다. 구글은 이미 동작 중이라
> 그쪽 설정은 이미 끝나 있을 가능성이 높습니다 — **카카오만 새로 하면 됩니다.**
...
**① https://console.cloud.google.com** 접속 → 프로젝트 만들기 (이름 `sado-toji`, GA 연동에 쓰는 것과 같은 프로젝트를 써도 됩니다)
...
> **모바일로 시도하다 막혔던 적이 있습니다 (2026-08-31).**
...
>
> 구글 로그인이 이미 동작하므로 그동안은 **막힌 일이 아니라 미룬 일**이었습니다.
> 이제 진행합니다.
...
`www.toji.fyi` 로도 들어오는 경우가 있다면 그것도 같이 등록하세요.
옛 주소(`sado-toji.vercel.app`)는 이미 죽었으니 등록할 필요 없습니다.
...
| 항목 | 값 |
|---|---|
| Site URL | `https://toji.fyi` |
| Redirect URLs | `https://toji.fyi/**` |

이걸 안 하면 로그인 후 엉뚱한 곳으로 튕깁니다. **구글 로그인이 이미
`toji.fyi` 에서 동작하고 있다면 이 값은 이미 맞게 들어가 있을 가능성이
큽니다** — 새로 만들 필요 없이 화면에서 한 번 확인만 하면 됩니다.
