...
(async () => {
  ...
  const LANDUSE = { type: 'FeatureCollection', features: [
    ...
    // 아직 임야인데 도로구역 '포함' — 편입될 땅이다. 남겨야 한다.
    luFeat('2', '산138-22임', RLAT + 0.0002, RLON + 0.002,
           'UFM200,UIA100', '준보전산지,도로구역(57호선)', '3,1', '접함,포함'),
    // **'저촉' 은 걸쳤을 뿐 도로가 아니다** (지시: "항상 포함만 넣습니다").
    luFeat('5', '산77임', RLAT + 0.0005, RLON + 0.002,
           'UIA100,UQB100', '도로구역(57호선),계획관리지역', '2,1', '저촉,포함'),
  ...
  check('지시2: 아직 임야여도 도로구역 포함이면 남긴다 (편입 대상)',
        !!byLabel['산138-22임'] && byLabel['산138-22임'].r === 0,
        JSON.stringify(byLabel['산138-22임'] || null));
  /* **저촉은 도로가 아니다** (2026-09-16 지시: "항상 포함만 넣습니다").
     법으로는 그 필지의 일부가 도로구역에 걸린 것이 맞지만, 걸렸다는 것과
     도로가 된다는 것은 다른 말이다. 라이브에서 산 하나가 통째로 칠해졌다. */
  check("저촉은 뺀다 — 걸쳤을 뿐 도로가 아니다",
        !byLabel['산77임'], Object.keys(byLabel).join(','));
  ...
  check('저촉 구분을 싣는다 (남는 것은 늘 포함이다)',
        (byLabel['산138-22임'] || {}).c === '포함',
