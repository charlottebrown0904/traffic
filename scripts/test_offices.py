...
trades = pd.DataFrame([
    ...
    for i, (cd, nm, umd, lat, lon) in enumerate([
        ...
        # **인구가 없는 시군구.** 2025년에 새로 생긴 화성시 4개 구가
        # 그렇다 — 실거래에는 41591 로 오는데 KOSIS 인구에는 그 코드가
        # 아직 없다. 예전에는 이 시군구가 regions.json 에서 통째로
        # 빠졌고, 그래서 땅값 지도에서 계획관리 거래의 4.5% 가 소리
        # 없이 사라져 있었다. (run 46 에서 드러났다)
        ("41591", "화성시 병점구", "병점동", 37.207, 127.032),
    ])
])
...
check(len(regions) == 4, f"네 시군구가 다 나온다 ({len(regions)})")
# **인구가 없다고 시군구를 빼면 안 된다.** 지도의 모든 겹이 그 시군구를
# 잃는다. 인구는 비우되 자리는 남긴다 — 인구 원은 그 해 값이 없으면
# 알아서 안 그리므로(app.js drawPopulation) 빈 pop 은 안전하다.
check("화성시 병점구" in by_name,
      "인구가 없어도 좌표가 있으면 남는다 (화성시 신설 구)")
check(by_name.get("화성시 병점구", {}).get("pop") == {},
      f"인구는 지어내지 않고 비워 둔다 — {by_name.get('화성시 병점구', {}).get('pop')!r}")
check(by_name["수원시 장안구"]["pop"].get("2025") == 280000,
      "인구가 있는 곳은 그대로 실린다")
