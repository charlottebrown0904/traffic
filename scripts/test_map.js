...
(async () => {
  ...
  try {
    ...
    check('지역별 가격에도 물음표가 없다', why.priceDots === 0,
          `${why.priceDots}개 남음`);
    // 관리지역이 왜 따로 있는지(2006~2010년의 잔재)를 여기서 보던 검사다.
    // 2026-09-17 에 '지역별 가격' 의 묶음 설명을 지우면서 이 자리에서는
    // 없어졌는데, **그 말 자체는 안 사라졌다** — 용도지역 칸
    // '관리(세분 없음)' 에 물음표로 붙어 있고 출처는
    // src/redt/webexport.py 의 LANDPRICE_ZONE_NOTE 다.
    //
    // 그러니 '없는지' 가 아니라 **제자리에 있는지**를 본다. 묶음 설명에서
    // 지워진 것은 맞고(왼쪽), 칸에는 그대로여야 한다(오른쪽) — 수집이
    // 그 문장을 떨어뜨리면 여기서 걸린다.
    check('묶음 설명에서는 빠졌다', !/2006~2010/.test(why.text));
    // 붙어 있는 자리는 **내보내기**다. 화면 쪽은 고정 표본이 주는 것만
    // 그리므로 여기서 보면 내가 표본에 넣은 것을 내가 확인하는 꼴이 된다
    // (칸에 사유가 붙고 눌러 읽히는 장치 자체는 아래 '용도지역 사유'
    // 절이 이미 본다). 그래서 **원본**을 본다 — 수집이 이 문장을
    // 떨어뜨리면 화면에서도 사라지고, 그때 여기서 걸린다.
    {
      const wx = fs.readFileSync(path.join(ROOT, 'src', 'redt', 'webexport.py'), 'utf8');
      check('관리지역 사유는 내보내기가 여전히 싣는다',
            /LANDPRICE_ZONE_NOTE/.test(wx) && /2006~2010년 신고분/.test(wx)
            && /관리\(세분 없음\)/.test(wx), '');
    }
    ...
    // 2026-09-13 지시(슬라이드 3): '교통량 순위' → 'IC 교통량', '추이 비교' 탭은
    // 접고 표의 '지가 추이' 단추가 팝업으로 연다. 화면은 그대로 있다.
    check("탭은 지도·매물·IC 교통량이 선다",
          tabs.shown.join(',') === '지도,매물,IC 교통량',
          tabs.shown.join(',') || '없음');
    check('추이 비교·스코어보드·가설 판정이 접혀 있다',
          tabs.hiddenViews.join(',') === 'trend,board,verdict',
    ...
    // 지역요인 (2026-09-13): 개별공시지가로 추정하는 코드는 있지만 **기본은 끈다** —
    // 전국 254건에서 ±30% 적중이 켠 채 78 · 끈 채 83 이었다. 그래서 1.000 이고
    // 사유는 평가서 관행(414/414 이 1.00)을 말한다. 150,000 × 1.03 × 1.000 × 1.243 × 2.32 = 445,580 → 446,000.
    check('지역요인은 1.000 — 평가서 관행을 사유로 적는다',
          /지역요인 비교[\s\S]{0,80}1\.000/.test(vcalc) && /414\/414/.test(vtxt),
          (vcalc.match(/지역요인 비교[\s\S]{0,200}/) || ['없음'])[0]);
  ...
  console.log('\n배율 정책 — docs/map-zoom-levels.md 와 코드가 같은가');
  {
     ...
     *
     *   "계획은 살려 놓고 실제로 표시는 계획 도로처럼 필지 기준으로
     *    선택될 수 있도록 방법을 전환바랍니다."
     *
     * 선은 아무리 잘 스냅해도 필지와 무관하다. 가까이서는 선 둘레 30m
     * 안의 필지를 면으로 그린다. 30m 는 고른 값이 아니라 잰 값이다
     * (scripts/road_parcel_probe.py: 30m 7~14개 · 50m 12~48개). */
    check('가까이서는 필지를 면으로 부른다',
          /mode=roadparcels/.test(app), '');
    check('그 배율 아래로는 안 부른다 (칸마다 열 번씩 부르지 않게)',
          /map\.getZoom\(\) >= ROADP_MIN_ZOOM/.test(app)
          && /const ROADP_MIN_ZOOM = 15/.test(app), '');
    /* **이것이 요점이다.** 신설 구간 밑은 아직 임야·전·답이고, 그 땅이
       편입될지가 땅 주인에게 가장 중요하다 (안성 서운면 실측: 산138-22임).
       이미 도로인 땅과 같은 색으로 칠하면 그 말이 사라진다. */
    check('이미 도로인 땅과 아직 아닌 땅을 갈라 칠한다',
          /p\.r === 1/.test(app) && /아직 도로가 아닙니다/.test(app), '');
    check('보상 시기·범위는 고시로 확인하라고 적는다 (단정하지 않는다)',
          /사업시행자 고시로 확인/.test(app), '');
    /* **'30m 안' 이라고 적으면 안 된다.** 이제 근사가 아니라 도로구역이
       판정한다 — 화면 문구가 옛 방식을 말하면 사용자가 그렇게 읽는다. */
    check("말풍선이 '30m 안' 이 아니라 도로구역을 말한다",
          /도로구역에 들어 있어 편입 대상/.test(app)
          && !/노선 30m 안이라 편입될 수 있습니다/.test(app), '');
    check('어느 도로구역인지 이름을 적는다 (노선이 붙어 온다)',
          /p\.z \|\| '도로구역'/.test(app), '');
    /* 면이 주인공이 되는 배율에서 선을 그대로 굵게 두면 '안 맞는 선' 이
       또 눈에 띈다. 그렇다고 지우면 면이 오기 전에 화면이 빈다. */
    check('가까이서는 선을 흐리게 남긴다 (지우지도, 굵게 두지도 않는다)',
          /const close = map\.getZoom\(\) >= ROADP_MIN_ZOOM/.test(app)
          && /weight: close \? 2 : 4/.test(app), '');
