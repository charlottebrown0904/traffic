...
(async () => {
  ...
  try {
    ...
    // 지역요인 (2026-09-13): 대상 개별공시지가 250,000 ÷ (표준지 150,000 × 개별요인
    // 1.243) = 1.341. 같은 지목군(전·답)이고 울타리(0.5~2.0) 안이라 그대로 곱한다.
    // 150,000 × 1.03 × 1.341 × 1.243 × 2.32 = 597,470 → 자리수 규칙으로 597,000.
    // (지역요인이 1.000 이던 때는 446,000 이었다.)
    check('지역요인을 개별공시지가로 추정해 적는다 (1.341)',
          /지역요인 비교[\s\S]{0,80}1\.341/.test(vcalc) && /개별공시지가 250,000/.test(vtxt),
          (vcalc.match(/지역요인 비교[\s\S]{0,160}/) || ['없음'])[0]);
    check('결정단가를 자리수 규칙으로 낸다 (597,000원/㎡)', /597,000원\/㎡/.test(vcalc),
