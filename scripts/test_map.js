...
(async () => {
  ...
  try {
    ...
    // 1.243) = 1.34. 같은 지목군(전·답)이지만 **위로는 올리지 않는다** (상한 1.0 —
    // 전국 254건 실측에서 올리는 쪽이 적중을 깎았다). 그래서 1.000 에 걸리고
    // 사유에 계산과 상한을 적는다. 150,000 × 1.03 × 1.000 × 1.243 × 2.32 = 445,580 → 446,000.
    check('지역요인은 개별공시지가 비율을 적되 위로는 1.000 에서 멈춘다',
          /지역요인 비교[\s\S]{0,80}1\.000/.test(vcalc) && /개별공시지가 250,000/.test(vtxt) && /상한 1\.00/.test(vtxt),
