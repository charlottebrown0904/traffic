...
print("43. 전력 파일 적재 — 이름을 코드에 잇고, 못 이은 것은 센다")
check(_IND.sido_key("강원특별자치도") == "강원" and _IND.sido_key("전라남도") == "전남"
      and _IND.sido_key("충청북도") == "충북",
      "시도 이름이 해마다 바뀌어도 앞 두 글자로 견준다")
check(_IND.sido_key("전남광주통합특별시") == "전남광주", "통합특별시는 따로 둔다")
_cm = {("경기", "안성시"): "41550", ("경기", "수원시팔달구"): "41115", ("강원", "강릉시"): "51150"}
check(_IND.resolve(_cm, "강원특별자치도", "강릉시") == "51150", "새 시도 이름으로도 잇는다")
check(_IND.resolve(_cm, "경기도", "팔달구") == "41115", "'팔달구' 와 '수원시팔달구' 를 같은 것으로 본다")
check(_IND.resolve(_cm, "경기도", "없는시") is None, "없는 이름은 None — 지어내지 않는다")
# 첫 적재(run 34807148446)에서 못 이은 셋 — 시도 세대 차 · 세종 빈 칸 · 구 있는 시
_cm2 = {("전남광주", "여수시"): "12130", ("전남광주", "광산구"): "12290", ("세종", ""): "36110",
        ("경기", "수원시장안구"): "41111", ("경기", "수원시권선구"): "41113", ("경기", "수원시팔달구"): "41115",
        ("인천", "미추홀구"): "28177"}
check(_IND.resolve(_cm2, "전라남도", "여수시") == "12130" and _IND.resolve(_cm2, "광주광역시", "광산구") == "12290",
      "'전라남도·광주광역시' 로 온 이름을 통합특별시 코드에 잇는다")
check(_IND.resolve(_cm2, "세종특별자치시", "nan") == "36110", "세종은 시군구 칸이 비어도 잇는다")
check(_IND.resolve(_cm2, "경기도", "수원시") == "41110", "구가 있는 시는 시 코드(앞 넷 + 0)로 잇는다")
check(_IND.resolve(_cm2, "인천광역시", "남구") == "28177", "옛 이름(남구)을 지금 이름(미추홀구)으로 잇는다")
_cm2[("대구", "군위군")] = "27720"
check(_IND.resolve(_cm2, "경상북도", "군위군") == "27720" and _IND.resolve(_cm2, "충청남도", "연기군") == "36110",
      "시도를 옮긴 곳(군위군→대구 · 연기군→세종)을 잇는다")
_pf1 = _pd.DataFrame({"시도": ["경기도"] * 3, "시군구": ["안성시"] * 3, "법정동": ["공도읍", "미양면", "공도읍"],
                      "년도": ["2025"] * 3, "월": ["01", "01", "02"], "전체호수": ["10", "5", "10"],
                      "전력사용량": ["1000", "500", "1200"]})
_r1, _d1 = _IND.power_rows("15104908", _pf1, _cm)
check(len(_r1) == 2 and _r1[0][:3] == ("41550", "202501", 1500.0),
      f"법정동을 시군구·월로 모은다 ({_r1[0][:3] if _r1 else None})")
_pf2 = _pd.DataFrame({'"조회기간"': ["2016-10"], '"시도"': ["경기도"], '"시군구"': ["안성시"],
                      '"용도업종(소)"': ["제조업"], '"판매량"': ["12,345"]})
_r2, _ = _IND.power_rows("15069678", _pf2, _cm)
check(_r2 == [("41550", "201610", 12345.0, "power_kwh_use:제조업", "kWh", "data.go.kr 15069678")],
      "따옴표 열 이름·쉼표 숫자·YYYY-MM 을 다 푼다")
_pf3 = _pd.DataFrame({"조회기간": ["2020-10-16"], "시도": ["경기도"], "시군구": ["없는시"],
                      "계약종별(대)": ["산업용"], "판매량": ["1"]})
_r3, _d3 = _IND.power_rows("15069679", _pf3, _cm)
check(_r3 == [] and _d3["unmatched"] == {"경기도 없는시": 1},
      "코드에 못 이은 이름은 버리되 **세어서** 돌려준다")
try:
    _IND.power_rows("15104908", _pd.DataFrame({"엉뚱": [1]}), _cm)
    check(False, "열이 다르면 터진다")
except ValueError:
...
# 시도 이름 줄이기 — 대조표는 '충북' 을 쓰는데 KOSIS 는 '충청북도' 로 준다.
_keys = {s: _IND._sido_key(s) for s in
         ("충청북도", "전라남도", "경상북도", "경기도", "서울특별시",
          "강원특별자치도", "세종특별자치시", "인천광역시")}
check(_keys == {"충청북도": "충북", "전라남도": "전남", "경상북도": "경북",
                "경기도": "경기", "서울특별시": "서울", "강원특별자치도": "강원",
                "세종특별자치시": "세종", "인천광역시": "인천"},
      f"시도 이름을 대조표 열쇠로 줄인다 ({_keys})")

# 본청 행 — 시도가 제 이름으로 앉은 행은 시도 코드 두 자리로 간다.
# 버리면 광역시·도가 걷는 취득세가 통째로 사라진다.
_recs2, _d2 = _IND.kosis_tax_rows(
    [{"C1_NM": "경기도", "C2_NM": "취득세", "PRD_DE": "2023", "DT": "7"}],
    "경기도", _cm, {"경기": "41"})
check(len(_recs2) == 1 and _recs2[0][0] == "41",
      f"시도 본청 행은 시도 코드 두 자리로 싣는다 ({_recs2})")

# 통합된 시군구는 잇고, 갈라진 곳은 안 잇는다 — 하나를 골라 이으면
# 그 구의 옛 값이 실제보다 커진다.
_cm3 = {("충북", "청주시"): "43110"}
_recs3, _ = _IND.kosis_tax_rows(
    [{"C1_NM": "청원군", "C2_NM": "재산세", "PRD_DE": "2013", "DT": "3"}],
    "충청북도", _cm3, {})
check(len(_recs3) == 1 and _recs3[0][0] == "43110",
      f"통합된 청원군은 청주시로 잇는다 ({_recs3})")
check(("인천", "중구") not in _IND.KOSIS_SGG_ALIAS
