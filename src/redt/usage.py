...
# ────────────────────────────────────────────────────────────────
#
# 요구사항(2026-09-07): "토지의 경우 개발 가능 지, 모양, 도로 접등의
...
# ── 실측이 이 설계의 절반을 고쳤다 (2026-09-07, 남이천·안성 반경 3km) ──
#
# 등급별 공시지가 중앙값(원/㎡). 세 지역(계획관리·생산관리·자연녹지)만.
#
#            남이천(농촌)              안성(도시 인접)
#   5 광대로     n<10                  216,100  (n=780)
#   4 중로       n<10                  259,600  (n=1,051)   ← 광대로보다 비싸다
#   3 소로      88,600  (n=1,221)      187,500  (n=1,152)
#   2 세로(가)   76,300  (n=2,946)     169,100  (n=7,240)
#   1 세로(불)   45,900  (n=530)       101,300  (n=2,291)
#   0 맹지      36,200  (n=1,567)      102,400  (n=1,992)   ← 세로(불)보다 비싸다
#
# **등급을 선형으로 넣으면 안 된다.** 안성에서 두 군데가 뒤집혔다. 광대로변
# 땅이 중로변보다 싸고, 맹지가 세로(불)보다 비싸다. 큰길에 접한 것이 늘
# 좋은 것은 아니다 — 소음·완충녹지·진출입 제약이 붙고, 맹지는 옆 필지와
# 합쳐 쓸 여지가 남는다. 등급은 **범주 더미**로 넣는다.
#
# 그런데 **자동차 진입 여부의 계단은 두 곳이 거의 같다.**
#
#   남이천   세로(불) 45,900 → 세로(가) 76,300   +66%
#   안성     세로(불) 101,300 → 세로(가) 169,100  +67%
#
# 농촌과 도시에서 +66%, +67%. 이 정도로 겹치는 것은 우연이 아니다. 값을
# 가르는 것은 도로 폭이 아니라 **차가 들어가느냐**다. 보고된 문제("도로를
# 접하는 가가 제일 중요합니다")이 여기서 정확히 맞는다.
#
# 그래서 헤도닉에 넣을 것은
#   road_car_ok   주 변수. 이진. 두 표본에서 같은 크기로 확인됐다
#   C(road_side)  등급은 순서를 버리고 범주로. 뒤집힘을 그대로 담는다
#   is_corner     각지
# road_grade() 는 화면 정렬·묶음에만 쓴다. 회귀에 선형으로 넣지 말 것.
...
def road_grade(road_side) -> int | None:
    ...
    **회귀에 선형 항으로 넣지 말 것.** 위 실측이 보여주듯 값이 등급을
    따라 고르게 오르지 않는다. 화면에서 정렬하거나 묶을 때만 쓴다.
