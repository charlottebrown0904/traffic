...
-- 가려진 지번을 되찾은 결과. **trade_parcel 과 다른 표다.**
--
-- trade_parcel 은 좌표로 맞춘 것이고, 그 좌표가 거짓이었다 — 국토부가
-- 지번 뒷자리를 가려서 주는데(계륵리 1**) 그것을 그대로 지오코더에 넘겨
-- 나온 점을 'parcel 단위' 라고 적어 왔다. 전국에서 재 보니 97.6%가 틀렸다.
--
-- 이 표는 면적으로 되찾은 것이다. 신고 면적은 토지대장 면적과 **같아야
-- 하는 값**이고 소수점까지 정확하다. 본번 범위·산여부·지목·용도지역으로
-- 좁힌 뒤, '한 필지는 같은 달에 한 번만 팔린다' 로 전체를 맞춘다.
--
-- 지분 거래는 들어오지 않는다. 신고 면적이 필지 전체가 아니라 판 지분의
-- 면적이라 면적으로 맞힐 수가 없다 (2026-09-19 지시).
--
--   method   area     낱개 필지 면적이 맞았다
--            bon_sum  거래 뒤에 갈라졌다 — 부번들의 합이 맞았다
--   n_parcel 합으로 맞춘 경우 그 본번 아래 필지 수. 1 이면 낱개다.
--
-- **여기 없는 거래는 화면에 올리지 않는다.** 위치를 모르는 것을 아는 척
-- 하면 남의 땅을 가리킨다.
CREATE TABLE IF NOT EXISTS trade_jibun (
    trade_id   VARCHAR PRIMARY KEY,
    pnu        VARCHAR,
    method     VARCHAR,
    n_parcel   INTEGER,
